 ...
 * @returns {string}
 */
function tempReportPath(reportName) {
    let reportId = _.uniqueId(reportName);
    let reportPath = path.resolve(path.join(os.tmpdir()), `./${reportId}.html`);
    return reportPath;
}
...
class ReportUtils {
     ...
     * @returns {Promise}
     */
    static toHtml(reportEntries, reportPath = tempReportPath('code-analyzer-report-')) {
        let htmlContent = REPORT_TEMPLATES.SIMPLE_HTML({
            'model': {
        ...
        return writeFileAsync(reportPath, htmlContent)
            .then(() => {
